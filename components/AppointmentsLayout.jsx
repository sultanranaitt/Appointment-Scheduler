...
const AppointmentsLayout = ({item}) => {
  const [changeAppointmentModal, setChangeAppointmentModal] = useState(false);
  ...
  return (
    <View style={styles.cardContainer}>
      ...
      <TouchableOpacity
        ...
        onPress={() => setChangeAppointmentModal(true)}>
        <Image
          source={require('../assets/images/change.png')}
          style={{width: 20, height: 20}}
          resizeMode="contain"
        />
        <Text style={{color: '#333'}}>Change Appointment Date</Text>
      </TouchableOpacity>
      <ChangeAppointmentModal
        changeAppointmentModal={changeAppointmentModal}
        setChangeAppointmentModal={setChangeAppointmentModal}
        id={item?.id}
      />
    </View>
  );
